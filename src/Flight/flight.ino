...
int kx = static_cast<int>(0.0004);      //Коэффициент комплементарного фильтра
int ky = static_cast<int>(0.0004);
...
int throttle = 0;   //Уровень тяги. 0-1023, ограничивается
...
void loop() {
    while (safetyKey) {
        ...
        //Получение сырых данных с акселерометра и расчет углов на из основы
        Acc = Accelerometer.readNormalize();

        TotalVector = sqrt(Acc.XAxis * Acc.XAxis + Acc.YAxis * Acc.YAxis +
                           Acc.ZAxis * Acc.ZAxis);  //Абсолютный вектор силы тяжести
        AccAngleX = asin(Acc.XAxis / TotalVector) *
                    -DEG;  //Углы рассчитываются из синуса угла между абс. Вектором и его проекцией на ось X или Y
        ...
        //приведение диапазона входных данных к диапазону работы регулятора оборотов бес коллекторного двигателя
    ...
    Serial.print("\n");*/
        ...
        //отправка сигнала на бесклеточный двигатель
