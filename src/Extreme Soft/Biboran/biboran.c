...
#define IMAGE   "res/abdul.png"
#define BIBORAN "res/biboran.txt"

#ifndef BOOL
typedef char BOOL;
#endif

BOOL READ_OPEN = FALSE; // Won't allow to create more than one read window.
...
string get_buffer_from_file(string filename) {
    string _string;
    string buffer;
...
string *window_init(int _w, int _h, _string title) {
...
void read_window_create(GtkWidget *p_wid, gpointer data) {
    ...
    DESTROY(read_win);
    DESTROY(scrolled_read);

    // Default icon.
    window_set_icon(read_win, ICON);

    // Read saint Biborant to buffer.
    string _string
    buffer;
    buffer = get_buffer_from_file(BIBORAN);
#ifdef DEBUG
    fprintf(stdout, "%s\n", buffer);
...
string *init_and_pack_button(GtkWidget *box, _string label) {
...
string *init_frame_with_shadow(_string label, GtkWidget *container) {
...
int main(int argc, char **argv) {
    ...
    // Abdul'
    GtkWidget *abdul;
    abdul = gtk_image_new_from_file(IMAGE);
